...
class Model(object):
    ...
    def evaluate(self, test_df):
        ...
        test_df["predict"] = test_df["content"].applymap(self.predict)
        print("start calculating")
        for cls in tqdm.tqdm(self.classes):
            real = test_df.loc[test_df["class"] == cls, "content"].values.tolist()
            predict = test_df.loc[test_df["predict"] == cls, "content"].values.tolist()
            real_n = test_df.loc[test_df["class"] != cls, "content"].values.tolist()
            predict_n = test_df.loc[test_df["predict"] != cls, "content"].values.tolist()
